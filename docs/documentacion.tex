...
disponibilidad continua mediante réplicas y sondas de vida.
...
\begin{tikzpicture}[node distance=1.1cm]
  ...
  \begin{scope}[on background layer]
    \node[contenedor, fit=(service)(pod1)(pod2),
...
\end{tikzpicture}
\caption{Arquitectura distribuida de 3 nodos con réplicas en MicroShift
(\texttt{replicas=2}, \texttt{livenessProbe} activo).}
...
\lstinputlisting[language=,caption={k8s/deployment.yaml --- 2 réplicas y sondas de salud.}]{../k8s/deployment.yaml}

\subsection{Manifiesto de Servicio --- service.yaml}
\lstinputlisting[language=,caption={k8s/service.yaml --- expone el servicio vía NodePort.}]{../k8s/service.yaml}
...
El sistema garantiza alta disponibilidad mediante tres capas:
...
chat-distribuido/
|-- src/
|   |-- main.ts              <- Punto de entrada, inicia en puerto 3000
...
|-- k8s/
|   |-- deployment.yaml      <- 2 replicas del servidor en MicroShift
|   `-- service.yaml         <- Expone el servicio (NodePort)
...
# 3. Desplegar en MicroShift
oc apply -f k8s/deployment.yaml
oc apply -f k8s/service.yaml

# 4. Esperar a que los pods esten listos (2/2 Running)
oc get pods -w
